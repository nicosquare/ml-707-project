...
class Microgrid:
    ...
    def service_provider_cost(self, c_t: float, coeff_a_t: float, coeff_p_t: float, prosumers_surplus: list, prosumers_shortage:list):
        ...
        sum_p_t += coeff_p_t * np.sum(prosumers_surplus) #shouldn't we check that we consider only prosumers' surplus?
        sum_a_t += coeff_a_t * np.sum(prosumers_shortage) #we are also not considering that prosumers might cover their consumption, rather we are selling more energy than prosuemrs need
